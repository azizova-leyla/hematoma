...
class PoolsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pool" do
    assert_difference('Pool.count') do
      post pools_url, params: { pool: { name: @pool.name, tournament_id: @pool.tournament_id } }
    end
  ...
  test "should update pool" do
    patch pool_url(@pool), params: { pool: { name: @pool.name, tournament_id: @pool.tournament_id } }
